  ...
  <view class="">
      ...
      <block v-if="item.type == 1">
        <view class="item-head flex item-center">
          <view
            ...
            @tap="userInfo(item.publishUser.userId)"
          >
          ...
          <view class="flex-1 head-msg">
            <view class="msg-box flex item-center">
              <view
                ...
                @tap="userInfo(item.publishUser.userId)"
                >{{ item.publishUser.userName }}</view
              >
              <view v-if="item.publishUser.militaryRankType" class="rank">
                <image
                  :src="
                    '/static/images/junxian/' +
                      item.publishUser.militaryRankType +
                      '.png'
                  "
                  mode="widthFix"
                />
              </view>
              <view v-if="item.publishUser.shopId" class="shop">
                <image :src="'/static/images/shop.png'" mode="widthFix" />
              </view>
              <view v-if="showNum" classs="numMark">{{ index }}</view>
            </view>
          ...
          <view
            class="followBox"
            v-if="!item.isOwner"
            @tap="guanzhu(item.publishUser.userCode, index)"
          >
        ...
        <block v-if="item.articleInfo.type == 2">
          <view class="articalBox">
            <view
              ...
              :class="{
                'clamp clamp-3':
                  !item.articleInfo.showMore && !item.articleInfo.isDetail
              }"
              @tap="linkDetail(item)"
            >
              <block v-if="item.articleInfo.showContent.length > 0">
                <block
                  v-for="(conitem, comind) in item.articleInfo.showContent"
                  :key="comind"
                >
            ...
            <view v-if="!item.articleInfo.isDetail">
              <text
                v-if="item.articleInfo.showMore"
                class="blue"
                @tap="hideMore(index)"
                >收起</text
              >
          ...
          <view class="imgList clearfix">
            <view
              v-for="(imgItem, imgInd) in item.articleInfo.imgList"
              :key="imgInd"
              class="imgItem flex-1"
            >
			  <view @tap="showBigImg(index, imgInd)" class="picBox" :style="'background:url('+imgItem+') center center no-repeat;background-size:auto 100%'"></view>
              <!-- <image
                :src="imgItem"
                mode="widthFix"
                @error="setErrorPic(index, imgInd)"
              ></image> -->
            </view>
          </view>
        </block>
        <block v-else-if="item.articleInfo.type == 1">
          <view
            ...
            v-if="item.articleInfo.imgList.length > 0"
          >
            <view class="a_pic">
              <image
                :src="item.articleInfo.imgList[0]"
                mode="widthFix"
                @tap="showBigImg(index, 0)"
              ></image>
            </view>
            <view class="a_main flex-1" @tap="linkDetail(item)">
              <view class="title blod clamp clamp-2 lh42">{{
                item.articleInfo.title
              }}</view>
              <view class="msg fs12 clamp clamp-2 lh42">{{
                item.articleInfo.zyao
              }}</view>
            </view>
          </view>
        ...
        <block v-for="(linkitem, linkInd) in item.itemLinkList" :key="linkInd">
          <view
            ...
            @tap="linkProd(linkitem.goods.goodsId)"
          >
            <view class="p_pic">
            ...
            <view class="p_main flex-1">
              ...
              <view class="price red fs12"
                >{{ linkitem.goods.priceSale }}元</view
              >
            </view>
            <view class="p_buy"
        ...
        <view class="itemOperBox flex">
          ...
          <button
            class="flex-1"
            type="share"
            :class="{ red: item.articleInfo.hasUp }"
            @tap="dianzan(item.articleInfo.id, index)"
          >
    ...
    <view v-if="nodata" class="noData flex f-row just-con-c item-center">
      <view class="text-center">
        ...
        <button class="radiuBtn" @tap="linktoshop" type="rednull">
          随便看看
        </button>
...
export default {
  ...
  methods: {
    ...
    linkDetail(obj, isComment) {
      uni.navigateTo({
        url: `/pages/home/commentDetail?data=${encodeURIComponent(
          JSON.stringify(obj)
        )}&pageType=${this.pageType}&isComment=${isComment ? isComment : ""}`
      });
    },
    linktoshop() {
      // debugger
