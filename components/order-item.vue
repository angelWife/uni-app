	...
	<view class="orderList">
		<view v-for="(item, index) in dataList" :key="index" class="item" @tap="orderDetail(item)">
			...
			<view v-if="orderType==100" class="item_body flex">
				<view class="pic" @tap.stop="productDetail(item.detail.goodsId)">
					<image :src="item.detail.goodsImgPath" mode="widthFix" />
				</view>
				<view class="flex-1">
					<view class="flex">
						<view class="name flex-1 clamp clamp-2"  @tap.stop="productDetail(item.detail.goodsId)">
							{{item.detail.goodsName}}
						</view>
						<view class="text-right"  @tap.stop="productDetail(item.detail.goodsId)">
			...
			<view class="orderBtn text-right">
				...
				<block v-if="item.status==5">
					<button type="rednull" v-if="item.flagStatusRefund !=1" @tap.stop="createOrder(item)"  >再来一单</button>
					<button type="null" v-if="item.flagStatusRefund !=1" @tap.stop="delOrder(item)" >删除订单</button>
				</block>
