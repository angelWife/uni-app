  ...
  <view class="content" id="myContent">
    <view class="detail listItem">
      <block v-if="dataInfo.type == 1">
        <view class="item-head flex item-center">
          <view
            class="img item-center comHeadPic"
            @tap="userInfo(dataInfo.publishUser.userId)"
          >
            <image
              class="headPic"
              :src="dataInfo.publishUser.imgPathHead"
            ></image>
            <image
              class="grade"
              src="../../static/images/baihu.png"
              mode="widthFix"
            ></image>
          </view>
          <view class="flex-1 head-msg">
            <view class="msg-box flex item-center">
              <view
                class="textEllipsis name fs15"
                @tap="userInfo(dataInfo.publishUser.userId)"
                >{{ dataInfo.publishUser.userName }}</view
              >
              <view v-if="dataInfo.publishUser.militaryRankType" class="rank">
                <image
                  :src="
                    '/static/images/junxian/' +
                      dataInfo.publishUser.militaryRankType +
                      '.png'
                  "
                  mode="widthFix"
                />
              </view>
              <view v-if="dataInfo.publishUser.shopId" class="shop">
                <image :src="'/static/images/shop.png'" mode="widthFix" />
              </view>
            </view>
          ...
          <view
            class="followBox"
            v-if="!dataInfo.isOwner"
            @tap="guanzhu(dataInfo.publishUser.userCode)"
          >
            <text class="follow" v-if="dataInfo.publishUser.hasFollow"
              >已关注</text
            >
        ...
        <block v-if="dataInfo.articleInfo.type == 2">
          <view class="articalBox">
            <view class="msg fs15 lg42" @tap="linkDetail(item)">
              <block v-if="dataInfo.articleInfo.showContent.length > 0">
                <block
                  v-for="(conitem, comind) in dataInfo.articleInfo.showContent"
                  :key="comind"
                >
                  <block v-if="conitem.type == 1">
                    <text class="name blue" @tap.stop="linkUser(conitem.atId)"
                      >@{{ conitem.atName }}</text
                    >
                  ...
                  <block v-else>
                    <text
                      class="from blue"
                      @tap.stop="linkTheme(conitem.topicId)"
                      >#{{ conitem.topicName }}#</text
                    >
                    <text class="text">{conitem.content}}</text>
                  </block>
                </block>
          ...
          <view class="imgList clearfix">
            <view
              v-for="(imgItem, imgInd) in dataInfo.articleInfo.imgList"
              :key="imgInd"
              class="imgItem flex-1"
            >
              <view
                class="picBox"
                @tap="showBigImg(index, imgInd)"
                :style="'background:url(' + imgItem + ') center center no-repeat;background-size:auto 100%'"></view>
        ...
        <block v-else>
          <view
            class="articalBox news flex item-center"
            v-if="dataInfo.articleInfo.imgList.length > 0"
          >
            <view class="a_pic">
              <image
                :src="dataInfo.articleInfo.imgList[0]"
                mode="widthFix"
              ></image>
            </view>
            <view class="a_main flex-1">
              <view class="title fs16 lg42 blod clamp clamp-2">{{
                dataInfo.articleInfo.title
              }}</view>
              <view class="msg fs15 lg42">
        ...
        <block
          v-for="(linkitem, linkInd) in dataInfo.itemLinkList"
          :key="linkInd"
        >
          <view
            v-if="linkitem.type == 1"
            class="adventBox shopProduct flex item-center"
            @tap="linkProd(linkitem.goodsId)"
          >
            <view class="p_pic">
            ...
            <view class="p_buy"
              ><button size="mini" type="red" class="">购买</button></view
            >
          </view>
          <view
            v-else
            class="adventBox ranking flex item-center"
            @tap="linkRanking(linkitem.rankType)"
          >
            <view class="p_pic">
              <image
                src="/static/images/defaultpro.png"
                mode="widthFix"
              ></image>
            </view>
            <view class="p_main flex-1">{{ linkitem.name }}</view>
            <view class="p_buy"
              ><button size="mini" type="red" class="radiuBtn">
                去看看
              </button></view
            >
          </view>
        ...
        <view class="operBox clearfix">
          ...
          <view class="item">
            <view
              class="mybtn"
              :class="{ item_red: dataInfo.articleInfo.hasUp }"
              @tap="dianzan"
            >
              <icon class="iconfont icon-dianzan"></icon>
              <text>{{ dataInfo.articleInfo.numTotalUp }}</text>
      ...
      <block v-else-if="dataInfo.type == 2">
        ...
        <view class="item-head flex item-center">
          ...
          <view class="flex-1 head-msg">
            ...
            <view class="timer c999 fs12">{{
              dataInfo.adInfo.createTime
            }}</view>
          </view>
        ...
        <view class="adventPicBox">
          <view class="adventPic">
            <image
              src="http://www.mypcera.com/star/mm/uploadfile/201005/4/A142330696.jpg"
              mode="widthFix"
            ></image>
          </view>
          <view class="text-right addventBtn"
            ><button size="mini" type="red">立即查看</button></view
          >
        </view>
      </block>
    </view>
          ...
          <view class="flex-1">
            ...
            <view class="oper flex item-center">
              ...
              <view class="operIocn c999" @tap="commentDetail(item)">
                回复{{
                  item.numTotalReply > 0 ? "(" + item.numTotalReply + ")" : ""
                }}
              </view>
              <view
                class="operIocn clearfix"
                :class="{ red: item.hasUp }"
                @tap="commentDianzan(item.id, ind)"
              >
                <icon class="iconfont icon-dianzan"></icon>
    ...
    <view class="operFoot flex">
      ...
      <view class="oper flex f-col" @tap="commentClick">
        ...
        <text>评论</text>
        <text v-if="dataInfo.articleInfo.numTotalComment > 0" class="mark">{{
          dataInfo.articleInfo.numTotalComment
        }}</text>
      </view>
      <view
        class="oper flex f-col"
        :class="{ red: dataInfo.articleInfo.hasUp }"
        @tap="dianzan"
      >
        <icon class="iconfont icon-dianzan"></icon>
        <text>赞</text>
    ...
    <commentModal
      :showModal="showModal"
      @relaseComment="relaseComment"
      @cancelRelase="cancelRelase"
    ></commentModal>
...
export default {
  ...
  onShareAppMessage(res) {
    ...
    if (self.dataInfo.articleInfo.type == 1) {
      ...
      settings.pagePath = `/pages/home/commentDetail?data=${encodeURIComponent(
        JSON.stringify(self.dataInfo)
    ...
    getCommentList() {
      ...
      self.$acFrame.HttpService.commentList(params).then(res => {
        if (res.success) {
          ...
          _row.filter(v => {
            v.militaryRankType = self.$acFrame.Util.setRankName(
              v.militaryRankType
            );
          });
